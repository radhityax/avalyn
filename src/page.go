...
func editPage(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if p.Pass != "" {
			if !checkPasswordHash(p.Pass, currentPassword) {
				err = db.QueryRow(`SELECT id, date, author, title, slug, content, status FROM posts WHERE slug=?`, slug).
					Scan(&p.ID, &p.Date, &p.Author, &p.Title, &p.Slug, &p.Content, &p.Status)
				if err != nil {
					http.NotFound(w, r)
					return
				}
